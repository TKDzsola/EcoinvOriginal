...
namespace Ecoinv.BL
{
    ...
    // (EUSERS mintára, MINDEN DB MEZŐ PROPERTY)
    // ======================================================================
    public partial class INVOICE_DETAILS : TableBaseClass
    {
        ...
        // ==========================================================
        // ================= UI / LOGIKAI MEZŐK =====================
        // (NEM adatbázis oszlopok!)
        // ==========================================================

        // ---- szolgáltatás neve (JOIN)
        [DebuggerBrowsable(DebuggerBrowsableState.Never)]
        ...
        // ---- nettó sor összeg (számolt)
        [DebuggerBrowsable(DebuggerBrowsableState.Never)]
        private decimal __line_total_net;
        public decimal LINE_TOTAL_NET
        {
            get => __line_total_net;
            private set => SetPropertyValue(nameof(LINE_TOTAL_NET), ref __line_total_net, value);
        }
        ...
        {
            get => __vat_amount;
            private set => SetPropertyValue(nameof(VAT_AMOUNT), ref __vat_amount, value);
        }

        // ---- bruttó sor összeg (számolt)
        [DebuggerBrowsable(DebuggerBrowsableState.Never)]
        private decimal __line_total_gross;
        public decimal LINE_TOTAL_GROSS
        {
            get => __line_total_gross;
            private set => SetPropertyValue(nameof(LINE_TOTAL_GROSS), ref __line_total_gross, value);
        }
    ...
    public partial class INVOICE_DETAILSTable
    {
        private readonly string selectSQL =
            "SELECT ID, INVOICEHEADERS_ID, SERVICES_ID, VATRATE_ID FROM INVOICE_DETAILS";

        private readonly string insSQL =
            "INSERT INTO INVOICE_DETAILS (ID, INVOICEHEADERS_ID, SERVICES_ID, VATRATE_ID) " +
            "VALUES ({0}, {1}, {2}, {3})";

        private readonly string delSQL =
            "DELETE FROM INVOICE_DETAILS WHERE ID = {0}";

        private readonly string selGenSQL =
            "SELECT GEN_ID(GEN_INVOICE_DETAILS_ID, 1) FROM RDB$DATABASE";
        ...
        private int GetGenerator(FBConnectX conn) =>
            DBFunc.Get_Generator(selGenSQL, conn);
