...
namespace Ecoinv.BL.Enums
{
  public partial class ADRESSES : TableBaseClass
  {
    ...
    public string ADDRESSES
    {
      ...
      set
      {
        ...
        SetPropertyValue(nameof(ADDRESSES), ref __addresses, value);
        OnADDRESSESChanged();
      }
    }
  ...
  {
    private readonly string selectSQL = "select id, client_id, city, addresses, atype, aactive, postalcode from adresses";
    private readonly string selectClntAdrSQL = "select id, client_id, city, addresses, atype, aactive, postalcode from adresses where client_id={0}";
    private readonly string insSQL = "insert into adresses (city, addresses, atype, aactive, postalcode) values ({0},'{1}','{2}','{3}','{4}')";
    private readonly string delSQL = "delete from adresses where id = {0}";
    private readonly string updSQL = "update adresses set city='{0}', addresses='{1}', atype='{2}', aactive={3}  where id = {3}";
    ...
    public void ReUpdateADRESSES_M(ADRESSES oldadresses)
    {
      ...
      _actrec.ADDRESSES = oldadresses.ADDRESSES;
    ...
    public void Insert(ADRESSES src, FBConnectX conn)
    {
      ...
      var _inssql = string.Format(insSQL, _id, src.ID, src.CLIENT_ID, src.CITY, src.ADDRESSES, src.ATYPE, src.AACTIVE, src.POSTALCODE);
    ...
    public void Update(ADRESSES src, FBConnectX conn)
    {
      ...
      if (__innerList.IndexOf(_actrec) != -1)
      {
        var _updsql = string.Format(updSQL, src.ID, src.CLIENT_ID, src.CITY, src.ADDRESSES, src.ATYPE, src.AACTIVE, src.POSTALCODE);
